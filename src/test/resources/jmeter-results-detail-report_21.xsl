...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="titleReport" select="'Midas Performace Test Results'" />
	<xsl:param name="dateReport" select="'peihao.he'" />
		...
		<html>
			<head>
				...
				<style type="text/css">
					...
					display: block;
					display/* hide this
					definition from IE5/6 */: table-row;
					}
	...
	<xsl:template name="pagelist">
		<h2>Pages</h2>
		<table align="center" class="details" border="0" cellpadding="8"
			cellspacing="2" width="95%">
			<tr valign="top">
				<th>URL</th>
				<th># Samples</th>
				...
				<xsl:if test="preceding::*/@lb">
					...
					<xsl:variable name="middle">
						<xsl:for-each select="../*[@lb = current()/@lb]/@t">
							<xsl:sort data-type="number" order="descending" />
							<xsl:if test="position() = ceiling($count div 2)+1">
								<xsl:value-of select="number(.)" />
							</xsl:if>
						</xsl:for-each>
					</xsl:variable>
					<xsl:variable name="percent">
						<xsl:for-each select="../*[@lb = current()/@lb]/@t">
							<xsl:sort data-type="number" order="descending" />
							<xsl:if test="position() = ceiling($count div 9)">
								<xsl:value-of select="number(.)" />
							</xsl:if>
						</xsl:for-each>
					</xsl:variable>
					...
					<xsl:variable name="QPS" select="$count div (number($endTime)-number($beginTime)) * 1000" />
					<tr valign="top">
						<xsl:attribute name="class">
					<xsl:choose>
						...
						<td align="center">
							<xsl:call-template name="display-time">
								<xsl:with-param name="value" select="$middle" />
							</xsl:call-template>
						</td>
						<td align="center">
							<xsl:call-template name="display-time">
